...
export function Stage({
  ...
  // Post-session workflow launcher (teacher-narration only)
  const [showWorkflowLauncher, setShowWorkflowLauncher] = useState(false);
  ...
  const doSessionCleanup = useCallback(() => {
    ...
    // Surface workflow launcher after a Q&A session in teacher-narration mode
    const stageData = useStageStore.getState().stage;
    if (stageData?.educationMode === 'teacher-narration' && activeType === 'qa') {
      setShowWorkflowLauncher(true);
    }
  ...
  return (
    ...
    >
      ...
      {/* Post-session workflow launcher (teacher-narration only) */}
      <WorkflowLauncher
        topic={useStageStore.getState().stage?.name ?? ''}
        open={showWorkflowLauncher}
        onClose={() => setShowWorkflowLauncher(false)}
      />
    </div>
  );
}
