...
import { ConversationRenderer } from '../scene-renderers/conversation-renderer';
import { GameRenderer } from '../scene-renderers/game-renderer';
...
export function SceneRenderer({ scene, mode }: SceneRendererProps) {
  const renderer = useMemo(() => {
    switch (scene.type) {
      ...
      case 'conversation':
        if (scene.content.type !== 'conversation') return <div>Invalid conversation content</div>;
        return <ConversationRenderer content={scene.content} mode={mode} sceneId={scene.id} />;
      case 'game':
        if (scene.content.type !== 'game') return <div>Invalid game content</div>;
        return <GameRenderer content={scene.content} mode={mode} sceneId={scene.id} />;
      default:
