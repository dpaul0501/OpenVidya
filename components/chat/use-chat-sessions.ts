...
import { getModeConfig } from '@/lib/config/education-modes';
import type { EducationMode } from '@/lib/config/education-modes';

const log = createLogger('ChatSessions');

/** Return agent IDs appropriate for the current education mode. */
function resolveAgentIds(
  educationMode: EducationMode | undefined,
  settingsAgentIds: string[],
  fallback: string[],
): string[] {
  if (educationMode) {
    const cfg = getModeConfig(educationMode);
    if (cfg.defaultAgentId) return [cfg.defaultAgentId];
  }
  return settingsAgentIds.length > 0 ? settingsAgentIds : fallback;
}
...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  ...
  const resumeSession = useCallback(
    async (sessionId: string): Promise<void> => {
      ...
      try {
        ...
        const educationMode = currentState.stage?.educationMode;
  ...
  const sendMessage = useCallback(
    async (content: string): Promise<void> => {
      ...
      // Read all selected agent IDs from settings store, respecting education mode
      const settingsState = useSettingsStore.getState();
      const currentStageState = useStageStore.getState();
      const educationMode = currentStageState.stage?.educationMode;
      const agentIds: string[] = resolveAgentIds(
        educationMode,
  ...
  const startDiscussion = useCallback(
    async (request: DiscussionRequest): Promise<void> => {
      ...
      // Read all selected agent IDs from settings store, respecting education mode
      const settingsState = useSettingsStore.getState();
      const discussionStageState = useStageStore.getState();
      const discussionEducationMode = discussionStageState.stage?.educationMode;
      const agentIds: string[] = resolveAgentIds(
        discussionEducationMode,
        settingsState.selectedAgentIds ?? [],
        [agentId],
      );
      // Ensure the trigger agent is included (unless mode overrides to a single agent)
      if (!discussionEducationMode && !agentIds.includes(agentId)) {
      ...
      try {
        ...
        await runAgentLoop(
          sessionId,
          {
            ...
            educationMode: discussionEducationMode,
            apiKey: mc.apiKey,
            baseUrl: mc.baseUrl,
            model: mc.modelString,
            providerType: mc.providerType,
