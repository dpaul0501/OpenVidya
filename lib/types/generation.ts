...
export interface UserRequirements {
  ...
  language: 'en-US' | 'sa'; // Course language - critical for generation
  userNickname?: string; // Student nickname for personalization
  userBio?: string; // Student background for personalization
  webSearch?: boolean; // Enable web search for richer context
  educationMode?: import('@/lib/config/education-modes').EducationMode;
  curriculumContext?: string; // Pre-built NCERT/board scope string injected into prompts
  studentClass?: number | null; // CBSE class number for KB lookup filtering (null = all classes)
}
...
export interface SceneOutline {
  id: string;
  type: 'slide' | 'quiz' | 'interactive' | 'pbl' | 'conversation' | 'game';
  title: string;
  ...
  order: number;
  language?: 'en-US' | 'sa'; // Generation language (inherited from requirements)
  educationMode?: string; // Education mode (inherited from requirements, used by slide content generator)
 ...
 */
export interface GeneratedConversationContent {
  scenario: string;
  participants: Array<{
    id: string;
    name: string;
