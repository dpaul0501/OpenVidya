...
> {
  ...
  switch (outline.type) {
    ...
    case 'conversation':
      return generateConversationContent(outline, aiCall);
    case 'game':
      return generateGameContent(outline, aiCall);
    default:
      return null;
  }
}
...
function buildSlideModeContext(mode?: string): string {
  switch (mode) {
    case 'story-quest':
      return `## Story Quest Slide Layout Rules
This slide is part of a story-driven lesson. Apply these layout rules:
- **Story slides**: Large visual area (image/video placeholder) takes 60%+ of canvas. Title is the scene name ("The Auto-Rickshaw Brakes Hard"). Body text = 1–2 lines max — the story hook, not the explanation.
- **"Why?" slides**: Single bold question centred on canvas. No bullet points. Large font (36–40px). Minimal background.
- **Concept slides**: Formula or key statement prominent (large, centred or top). 2–3 short bullet points below. Reference the story: "In the auto-rickshaw: F = ma".
- **Application slides**: 2–3 image placeholders side by side with 1-line captions. Visual grid layout.
- All slides: visual-first, minimal text. The teacher narrates — the slide shows, not tells.`;

    case 'rapid-revision':
      return `## Rapid Revision Slide Layout Rules
This slide is a memory-trigger card, not a lesson slide:
- Title = concept name (large, top, bold)
- Body = exactly 2–3 bullet points, each ≤8 words (formula, key relationship, common error)
- Use large font (20–22px for bullets) — this is a quick-scan card
- No images unless a diagram is essential for the formula
- Background: clean, minimal — dark accent colour on title band works well`;

    case 'exam-dojo':
      return `## Exam Dojo Slide Layout Rules
This slide is a drill card:
- **Probable Q slides**: Prominent question text (18–20px), exam source tag small below ("CBSE 2023 | 3 marks"). No decoration.
- **Solution slides**: Step-by-step layout — numbered steps with formula highlighted. Trap exposed in a distinct coloured box at bottom.
- Use clean, high-contrast layout — student is in exam headspace`;

    case 'lab-without-walls':
      return `## Lab Without Walls Slide Layout Rules
This slide is a lab briefing card — minimal text, apparatus-focused:
- **Brief slides**: Title = experiment name exactly as in NCERT. 2–3 bullet points: objective, apparatus list, what to observe.
- Use a diagram or apparatus image if available — circuit, ray diagram, apparatus setup sketch
- Keep text very concise — Lab Sir explains verbally; the slide is the bench setup reference`;

    case 'teacher-narration':
      return `## Teacher Narration Slide Layout Rules
This slide is a spoken-word visual aid. The teacher narrates — the slide supports:
- keyPoints become short phrase-length bullets (≤8 words each)
- Include an image if one is available — teacher narrates around it
- Avoid dense text — if a keyPoint is a full sentence, shorten it to a phrase
- One clear visual hierarchy: title → 1 image or diagram → 2–4 short bullets`;
...
async function generateSlideContent(
  ...
  const slideModeContext = buildSlideModeContext(educationMode);
...
/**
 * Generate conversation content for speaking practice
 */
async function generateConversationContent(
  outline: SceneOutline,
  aiCall: AICallFn,
): Promise<GeneratedConversationContent | null> {
  log.info(`Generating conversation content for: ${outline.title}`);

  try {
    const prompt = buildPrompt(PROMPT_IDS.CONVERSATION_CONTENT, {
      title: outline.title,
      description: outline.description,
    ...
    if (!parsed || typeof parsed !== 'object') {
      log.error('Invalid conversation content response:', response);
      return null;
    }

    const content = parsed as GeneratedConversationContent;

    // Validate required fields
    if (!content.scenario || !Array.isArray(content.participants)) {
      log.error('Missing required fields in conversation content');
      return null;
    }

    log.info(`Conversation generated: ${content.participants.length} participants`);
    return content;
  } catch (error) {
    log.error(`Failed to generate conversation content:`, error);
    return null;
  }
}

/**
 * Generate game content for learning games
 */
async function generateGameContent(
  outline: SceneOutline,
  aiCall: AICallFn,
): Promise<GeneratedGameContent | null> {
  const gameConfig = outline.gameConfig || {
  ...
  log.info(`Generating game content for: ${outline.title}`);
  ...
  try {
    const prompt = buildPrompt(PROMPT_IDS.GAME_CONTENT, {
      ...
      challengeCount: gameConfig.challengeCount,
    ...
    if (!parsed || typeof parsed !== 'object') {
      log.error('Invalid game content response:', response);
      return null;
    }

    const content = parsed as GeneratedGameContent;

    // Validate required fields
    if (!content.gameType || !Array.isArray(content.challenges)) {
      log.error('Missing required fields in game content');
      return null;
    }

    log.info(`Game generated: ${content.challenges.length} challenges, type: ${content.gameType}`);
    return content;
  } catch (error) {
    log.error(`Failed to generate game content:`, error);
    return null;
  }
}
...
export async function generateSceneActions(
  ...
  if (outline.type === 'conversation' && 'scenario' in content) {
    // For conversation scenes, generate actions to start the dialogue
    return generateDefaultConversationActions(outline, content);
  }

  if (outline.type === 'game' && 'gameType' in content) {
    // For game scenes, generate actions to start the game
    return generateDefaultGameActions(outline, content);
  }
...
/**
 * Generate default Conversation Actions (fallback)
 */
function generateDefaultConversationActions(outline: SceneOutline, content: GeneratedConversationContent): Action[] {
  ...
  // Add a discussion action to enable conversation
  actions.push({
    id: `action_${nanoid(8)}`,
    type: 'discussion',
    title: outline.title,
    topic: content.scenario,
    prompt: `开始${content.scenario}的对话练习。`,
  });
...
/**
 * Generate default Game Actions (fallback)
 */
function generateDefaultGameActions(outline: SceneOutline, content: GeneratedGameContent): Action[] {
  const actions: Action[] = [];
  ...
  actions.push({
    ...
    type: 'speech',
    title: 'Start Game',
    text: `Let's start the ${content.gameType} game! You have ${content.challenges?.length || 0} challenges to complete.`,
  });
  ...
  if (outline.type === 'game' && 'gameType' in content) {
    const sceneResult = api.scene.create({
      ...
      content: {
        ...
        scoringSystem: content.scoringSystem,
      },
      actions,
    });
