...
import { isValidMode, DEFAULT_EDUCATION_MODE } from '@/lib/config/education-modes';
...
const initialFormState: FormState = {
  ...
  language: 'en-US',
  webSearch: false,
  educationMode: DEFAULT_EDUCATION_MODE,
};
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isValidMode(savedMode)) {
        updates.educationMode = savedMode;
      }
      // Workflow redirect: carry topic from previous lesson
      const workflowTopic = localStorage.getItem('workflowTopic');
      if (workflowTopic) {
        updates.requirement = workflowTopic;
        localStorage.removeItem('workflowTopic');
      }
      if (savedLanguage === 'en-US' || savedLanguage === 'sa') {
  ...
  const updateForm = <K extends keyof FormState>(field: K, value: FormState[K]) => {
    ...
    try {
      ...
      if (field === 'educationMode') localStorage.setItem(EDUCATION_MODE_STORAGE_KEY, String(value));
  ...
  const handleGenerate = async () => {
    ...
    // Navigate to lesson-setup with topic — mode, PDF, and URLs are picked there
    router.push(`/lesson-setup?topic=${encodeURIComponent(form.requirement.trim())}`);
  };
